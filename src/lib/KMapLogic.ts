...
function piToExpression(pi: string, varCount: number, isSOP: boolean): string {
  const vars = varCount === 2 ? ['A', 'B'] : varCount === 3 ? ['A', 'B', 'C'] : ['A', 'B', 'C', 'D'];
  let terms: string[] = [];
...
export function simplifyExpression(groups: KMapGroup[], isSOP: boolean): string {
  ...
  // For a simple simulator, we'll just join all prime implicants.
  // A more advanced one would solve the Set Cover problem (Petrick's Method).
  // But for 4 variables, showing all PIs is often what students want to see first.
  
  const expressions = groups.map(g => g.expression);
  // Remove duplicates
  const uniqueExpr = Array.from(new Set(expressions));
